...
> **DEPRECATED** — This file is no longer loaded as an agent prompt.
> All workspace rules now live in [`AGENTS.md`](./AGENTS.md), which is
> automatically loaded into every agent. Agent-specific system prompts in
> `opencode.json` now contain only identity, role, and task description.
> Do not edit this file; update `AGENTS.md` instead.
